...
/* Includes ------------------------------------------------------------------*/
#include "stm32f10x_flash_swil.h"
...
uint32_t internalTable[] =
{
   (uint32_t)&FLASH_SetLatency
  ,(uint32_t)&FLASH_HalfCycleAccessCmd
  ,(uint32_t)&FLASH_PrefetchBufferCmd
  ,(uint32_t)&FLASH_Unlock
  ,(uint32_t)&FLASH_Lock
  ,(uint32_t)&FLASH_ErasePage
  ,(uint32_t)&FLASH_EraseAllPages
  ,(uint32_t)&FLASH_EraseOptionBytes
  ,(uint32_t)&FLASH_ProgramWord
  ,(uint32_t)&FLASH_ProgramHalfWord
  ,(uint32_t)&FLASH_ProgramOptionByteData
  ,(uint32_t)&FLASH_EnableWriteProtection
  ,(uint32_t)&FLASH_ReadOutProtection
  ,(uint32_t)&FLASH_UserOptionByteConfig
  ,(uint32_t)&FLASH_GetUserOptionByte
  ,(uint32_t)&FLASH_GetWriteProtectionOptionByte
  ,(uint32_t)&FLASH_GetReadOutProtectionStatus
  ,(uint32_t)&FLASH_GetPrefetchBufferStatus
  ,(uint32_t)&FLASH_ITConfig
  ,(uint32_t)&FLASH_GetFlagStatus
  ,(uint32_t)&FLASH_ClearFlag
  ,(uint32_t)&FLASH_GetStatus
  ,(uint32_t)&FLASH_WaitForLastOperation
  
  ,(uint32_t)&FLASH_UnlockBank1
  ,(uint32_t)&FLASH_LockBank1
  ,(uint32_t)&FLASH_EraseAllBank1Pages
  ,(uint32_t)&FLASH_GetBank1Status
  ,(uint32_t)&FLASH_WaitForLastBank1Operation
};
/**
  * @}
  */
